...
ruta = "./prueba/ciudades.csv"
tabla = "ciudades"

def intro_datos(tabla, ruta, sep = ";"):

  df_csv = pd.read_csv(ruta, sep = ";") # convierte el csv en un pd
  lista_dict = list(df_csv.keys()) # saca la indo de los nombres de las columnas pasandolas a una lista
  df_csv.set_index(columns =lista_dict, inplace= True) # quitamos el indice implicito y dejamos solo las columnas, no hay columna indice
